...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	else
	{
		...
		if(argc <= 4)
		{
			...
			cerr << "Usage : "<< (LPCTSTR)str << " [e(cho)|-a(lpha)|-s(pace)] TableFile InputFile OutputFile "<< endl;
		}
		else
		{
			...
			if(fpin && fpout)
			{
				while(!feof(fpin))
				{
					...
					{
						if( (!bNoAlpha && isprint(ch[0]))||(!bNoSpace && isspace(ch[0])))
						{
							if(ch[0] != '\r') //防止出现两个回车
								fprintf(fpout,"%c",ch[0]);
							bUnknown=false;
							TransCharNum++;
						}
						else if(!bUnknown)
